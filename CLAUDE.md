...
| `config-loader.ts` | WT/Ghostty config import. Reachable from Settings → Terminal → Import; before that the parsers, their IPC and their preload bindings existed with no caller, which is why the `background-opacity` they parse into `ThemeConfig.backgroundOpacity` was never applied to anything. WT spells opacity two ways — modern `opacity` (0-100, independent of `useAcrylic`) and pre-1.12 `acrylicOpacity` (0-1, only with `useAcrylic`) |
